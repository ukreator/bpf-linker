...
pub unsafe fn optimize(
    ...
    for instruction in mem_intrinsic_instructions.iter().copied() {
        ...
        let function_type = {
            let return_type = LLVMGetReturnType(function_type);
            let param_count = LLVMCountParamTypes(function_type);
            // Use the parameter types we're about to pass rather than the parameter types of the function.
            let mut param_types: Vec<_> = instruction_operands
                .iter()
                .copied()
                .map(|operand| LLVMTypeOf(operand))
                .collect();
            let is_var_arg = LLVMIsFunctionVarArg(function_type);
